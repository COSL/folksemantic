...
If you are running these tasks on the production server you will need to add 'RAILS_ENV=production'  
  ...
  rake muck:raker:db:populate RAILS_ENV=production
  rake muck:raker:db:bootstrap_services RAILS_ENV=production
...
Run the feed harvester.  This will download entries for each feed in the database and so it might take a while to run.
  rake muck:raker:harvest 

Run the recommender.  This will create recommendations for each of the entries in the database:
  rake muck:raker:recommend
