...
It assumes that you have a technical background, have never developed with Ruby on Rails, and are starting from ground zero on your machine. To develop and deploy Folksemantic you:
...
[Ruby on Rails](http://rubyonrails.org/) web application framework. Third party Ruby libraries are called Gems. Ruby Gems is the standard Ruby package manager, similar to apt-get, emerge, and other OS package managers.

Folksemantic is a web application written on top of the Ruby on Rails framework. Follow these [instructions on installing Ruby, Ruby on Rails, and Ruby Gems](http://rubyonrails.org/download) to get started. As of 10/3/08 the following versions are recommended: [Ruby 1.8.6-26](http://rubyforge.org/frs/download.php/29263/ruby186-26.exe), [Ruby Gems 1.3.0](http://rubyforge.org/frs/download.php/43986/rubygems-1.3.0.zip), and Ruby on Rails 2.1.1.
   ...
   4. Add github to the list of gem sources (some of the gems that Folksemantic uses are hosted on github):
...
Folksemantic utilizes a database on the back end. Development of Folksemantic has used [MySQL, the world's most popular open source database](http://www.mysql.com/), but it should be use other databases such as Postgres. Download and install [MySQL Community Server edition](http://dev.mysql.com/downloads/mysql/5.0.html#win32).
...
[Git](http://git.or.cz/) is a version control system like CVS or SVN (but better). Folksemantic developers use it to coordinate work. The [definitive source for Folksemantic](http://github.com/jbasdf/Folksemantic/tree/master) is hosted at [github](http://github.com/) (an awesome free service!).
...
GetText is a GNU translation utility used by Folksemantic to support translation into other languages. Download and install [GetText for Windows](http://gnuwin32.sourceforge.net/packages/gettext.htm), and the bundled utilities, so that you will have [msgmerge](http://gd.tuwien.ac.at/linuxcommand.org/man_pages/msgmerge1.html) for merging updated translation files into existing ones. 


=== Install RMagick=== 

[RMagick](http://rmagick.rubyforge.org/) is an image manipulation library that Folksemantic uses. Use these instructions to [Download and Install on Windows](http://rmagick.rubyforge.org/install-faq.html#win):
...
Now that you have installed all of that fun stuff, it is time to pull down Folksemantic. To do this, open a command prompt, go to your root projects folder and use the git clone command:

git clone git://github.com/jbasdf/Folksemantic.git

That will create a folder called Folksemantic and copy the source code to it. If you want to copy the source to a folder other than Folksemantic, just specify the folder name at the end:

git clone git://github.com/jbasdf/Folksemantic.git Folksemantic2

Now that you have pulled the code down, you just need to install the gems (ruby libraries) that Folksemantic uses, set up some configuration files, and set up a database for Folksemantic to use.
...
If you are setting up a new Folksemantic instance, you will need to obtain various keys to put in the configuration files. For example, your website will need its own reCaptcha key. To get that, register on the [reCaptcha website](http://recaptcha.net/) and request a key.


=== Install Folksemantic gem dependencies=== 

Folksemantic depends on the gems specified in config/environment.rb. Install them using the following [Rake](http://wiki.rubyonrails.org/rails/pages/WhatIsRakeAndWhatDoesItDo) command:
...
**Note**: Windows users need to load the win32console gem before using rake to load the Folksemantic schema. Use the following syntax to install the win32console-1.2.0-x86-mswin32-60 gem:
